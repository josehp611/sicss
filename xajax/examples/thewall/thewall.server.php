<?php
/*
 * File: thewall.server.php Example which demonstrates a xajax implementation of
 * a graffiti wall. Title: Graffiti Wall Example Please see <copyright.inc.php>
...
/*
 * String: DATA_FILE The file that will be used to store the messages on the
 * server.
 */
...
class graffiti {
	/*
	 * String: html Stores the html that is being generated for the current
	 * request.
	 */
	var $html;
	
	/*
	 * Boolean: isValid Indicates that the html generated for this request is
	 * valid and can be added to the wall <DATA_FILE>.
	 */
	var $isValid = false;
	
	/*
	 * Constructor: graffiti Builds the <html> output and sets the <isValid>
	 * indicator.
	 */
	...
	/*
	 * Function: getRandomFontSize Generate a font size based off a random
	 * number.
	 */
	...
	/*
	 * Function: getRandomColor Generate a browser safe color based on a random
	 * number.
	 */
	...
	/*
	 * Function: getRandomAlignment Generates a text-alignment value based on a
	 * random number.
	 */
	...
	/*
	 * Function: save Writes the current <graffiti->html> to the <DATA_FILE>
	 * when <graffiti->isValid> or returns an error message.
	 */
	function save() {
		if ($this->isValid) {
...
/*
 * Section: xajax request handlers Function: scribble Processes the users form
 * input and passes the values to an instance of the <graffiti> class. If the
 * graffiti class generates and error, it is returned to the browser using
 * <xajax->alert>, otherwise, <xajax->script> is used to instruct the browser to
 * make a request to <updateWall> and <xajax->clear> is used to clear the form
 * input.
 */
...
/*
 * Function: updateWall Processes the data previously written to the <DATA_FILE>
 * by <graffiti->save>.
 */
function updateWall() {
	$objResponse = new xajaxResponse ();
