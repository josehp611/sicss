<?php
/*
 * File: tableUpdater.inc.php Contains a class that can be used to invoke DOM
 * calls on the browser which will create or update an HTML table. Title:
 * clsTableUpdater class Please see <copyright.inc.php> for a detailed
...
class clsTableUpdater extends xajaxResponsePlugin {
	/*
	 * String: sDefer Used to store the state of the scriptDeferral
	 * configuration setting. When script deferral is desired, this member
	 * contains 'defer' which will request that the browser defer loading of the
	 * javascript until the rest of the page has been loaded.
	 */
	var $sDefer;
	
	/*
	 * String: sJavascriptURI Used to store the base URI for where the
	 * javascript files are located. This enables the plugin to generate a
	 * script reference to it's javascript file if the javascript code is NOT
	 * inlined.
	 */
	var $sJavascriptURI;
	
	/*
	 * Boolean: bInlineScript Used to store the value of the inlineScript
	 * configuration option. When true, the plugin will return it's javascript
	 * code as part of the javascript header for the page, else, it will
	 * generate a script tag referencing the file by using the
	 * <clsTableUpdater->sJavascriptURI>.
	 */
	var $bInlineScript;
	
	/*
	 * Function: clsTableUpdater Constructs and initializes an instance of the
	 * table updater class.
	 */
	...
	/*
	 * Function: configure Receives configuration settings set by <xajax> or
	 * user script calls to <xajax->configure>. sName - (string): The name of
	 * the configuration option being set. mValue - (mixed): The value being
	 ...
	 * the script generation phase. This will either inline the script or insert
	 * a script tag which references the <tableUpdater.js> file based on the
	 * value of the <clsTableUpdater->bInlineScript> configuration option.
	 */
