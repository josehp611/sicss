...
/*
 * Function: xajaxCompressFile <xajax> will call this function internally to
 * compress the javascript code for more efficient delivery. Parameters: $sFile
 * - (stirng): The file to be compressed.
 */
function xajaxCompressFile($sFile) {
	...
	$lines = explode ( "\n", $sFile );
	// loop through all the lines, building a new string at the same time as
	// removing literal strings
	$clean = '';
	$inComment = false;
	$literal = '';
	$inQuote = false;
	...
	for($i = 0; $i < $iLen; ++ $i) {
		...
		// loop through line's characters and take out any literal strings,
		// replace them with ___i___ where i is the index of this string
		$jLen = strlen ( $line );
		for($j = 0; $j < $jLen; ++ $j) {
			$c = substr ( $line, $j, 1 );
			$d = substr ( $line, $j, 2 );
			...
			if (! $inQuote && ! $inComment) {
				...
				if (($c == '"' || $c == "'") && ! $inComment && ! $inNormalComment) {
					...
					$literal = $d;
					$j ++;
				} else if ($d == "//") 				// ignore string markers that are found inside
				                  // comments
				{
