...
class ODBCIT {
	...
	public function GastoM($y,$cia,$gasto,$cc,$mes){
		$fecha1 = $y.$mes.'01';
		$fecha2 = $y.$mes.'31';
		$s2 = "Select SUM(PMVAL) From CTBTALDB.TCTPREMO Where PMCIA = '".$cia."' "
	...
	public function Existe($y,$cia,$cc){
		$s3 = "Select COUNT(*) From CTBTALDB.TCTPRESU Where PRCIA = '".$cia."' "
	...
	public function MontoPresupuesto($y,$cia,$gasto,$cc,$mes,$valor){
		...
		$campo = 'PREM'.$mes;
		$s4 = "Select ".$campo." From CTBTALDB.TCTPRESU Where PRCIA = '".$cia."' "
			." AND PRAÑO = '".$y."' AND PRCCO = '".$cc."' "
			." AND PRTABG = '".$gasto."'";
		$q4 = $this->ejecutar($s4);
		odbc_fetch_row($q4);
	...
	public function MontoAutorizado($y,$cia,$gasto,$cc,$mes,$valor){
		$s5 = "Select SUM(PAADIC) From CTBTALDB.TCTPREAU Where PACIA = '".$cia."' "
	...
	public function MontoDecremento($y,$cia,$gasto,$cc,$mes){
		$s5 = "Select SUM(PAREST) From CTBTALDB.TCTPREAU Where PACIA = '".$cia."' "
			." AND PAAÑO = '".$y."' AND PAMES = '".$mes."' "
			." AND PACCO = '".$cc."' "
			." AND PATABG = '".$gasto."'";
		$q5 = $this->ejecutar($s5);
	 ...
	 */
	 public function infoCheque($fecha,$cia,$gasto,$cc,$orden,$punt){
		...
		if ($punt == '1'){
			$s6 = "Select * From CTBTALDB.TCTPREMO Where PMCIA = '".$cia."'"
	...
	public function infoProvision($fecha,$cia,$gasto,$cc,$orden,$punt){
		$orden = str_pad($orden,10,' ',STR_PAD_LEFT);
		if ($punt == '1'){
			$s7 = "Select * From CTBTALDB.TCTPREMO Where PMCIA = '".$cia."'"
				." AND PMFECR = '".$fecha."' AND PMTIPD = 'OC'"
				." AND PMCCO = '".$cc."' "
				." AND PMNDOC = '".$orden."'";
		} else {
	...
	public function verCierre($fecha,$cia){
		...
		$mm = substr($fecha,4,2);
		$yyyymm = $yyyy.$mm;
		$s8 = "Select PUCERA From CTBTALDB.TCTPREPU Where PUCIA = '".$cia."'"
			." And PUCOD = 'CI'";
		$q8 = $this->ejecutar($s8);
		odbc_fetch_row($q8);
	...
	public function Dispo2($ciad,$ccd,$gastod,$mesd,$dispod) {
		...
		do {
			...
			$rest_aut=0;
			//  PRESUPUESTO
			$sq_d = "Select ".$fldpre.",".$fldgas." From CTBTALDB.TCTPRESU "
				."Where PRCIA='".$ciad."' And PRAÑO='".$apre
				."' And PRCCO='".$ccd."' And PRTABG='".$gastod."'";
			$qr_d = $this->ejecutar($sq_d);
			odbc_fetch_row($qr_d);
			$msgd2 .= "Presupuesto : Pre=".odbc_result($qr_d, 1)." Gas=".odbc_result($qr_d, 2)."\t";
			$suma += odbc_result($qr_d, 1);
			$resta += odbc_result($qr_d, 2);
			odbc_free_result($qr_d);
			// AUTORIZACIONES
			$sq_a = "Select PAADIC,PAREST From CTBTALDB.TCTPREAU "
			...
			$msgd2 .= "Autorizaciones : Adic=".$suma_aut." Dec=".$rest_aut."\n";
			if ($i >= $mprePU) {
				$fecha1 = $apre.$i2.'01';
				$fecha2 = $apre.$i2.'31';
				// GASTOS AUN NO CERRADOS
				$s2 = "Select SUM(PMVAL) From CTBTALDB.TCTPREMO Where PMCIA = '".$ciad."' "
	...
	public function ocuparaPresupuestoMesesAnteriores($cia,$cc,$gasto) {
		$bandera = 'N';
		$sq_ocupa = "Select CPSWIT From CTBTALDB.TCTPRECP Where CPCIA='".$cia
	...
	*/
	...
	public function verificaPresupuesto($cia,$titgas, $detgas) {
		$bandera = 'N';
		$sq_ocupa = "Select RSWIC From CTBTALDB.TTABGAS Where RCOMP='".$cia
