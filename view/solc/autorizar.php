...
$(function(){
	...
	$('#btnNegar').live('click', function(){
		...
		jConfirm('esta seguro?, la solicitud no podra utilizarse mas.', 'negar solicitud', function(answer){
			if(answer){
				var campos = new Object();
				campos['prehsol_numero'] = GetURLParameter('ps');
				campos['empresa'] = GetURLParameter('es');
...
<fieldset>
	<legend>
		<a href="?c=solc&a=autoriza&id=5&es=<?php echo $_GET['es']; ?>"><img alt="" src="images/back-black.png"></a>
		pre-solicitud de compra # <b><?php echo $_GET['ps']; ?></b>
		<input type="hidden" value="<?php echo $infohsol[0]['id_prehsol']; ?>" id="id_prehsol" name="id_prehsol">
		<strong><?php echo $conf->getEstado($infohsol[0]['prehsol_estado']); ?></strong>
		<?php if ($infohsol[0]['prehsol_estado'] == 1 && count($detas) > 0) { ?>
	...
	<?php if(count($detas) <=0 ) { ?>
		<div class="alert alert-warning">la solicitud esta vacia</div>
	<?php } ?>
	<table id="tablaPresol" name="tablaPresol" class="table table-condensed">
		...
		<?php foreach ($detas as $deta) { ?>
			<tr id="<?php echo $deta['id_predsol']; ?>">
				<td><?php echo $deta['predsol_cantidad']; ?></td>
				<td><?php echo $deta['predsol_descripcion']; ?></td>
			</tr>
			<?php $cantidades++; ?>
		<?php }?>
		</tbody>
