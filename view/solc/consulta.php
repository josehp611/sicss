...
function CheckAjaxCall(str) {
  ...
  $.ajax({
    ...
    success: function(aData){
		...
		if(postback){
			$('#centrocosto').val('<?php echo $id_cc?>');
			postback=false;
		}
...
$(function(){
	...
	//la lista de centros de costo
  	...
  	<?php if(!empty($id_empresa)): ?>
  		console.log('cc: <?php echo $id_empresa; ?>');
  		if('<?php echo $id_cc; ?>' != "") {
    		$('label#centros').after('<img src="images/ajax-loader.gif"></img>');
      		CheckAjaxCall('<?php echo $id_empresa; ?>');
    	} else {
      		$('#centrocosto').html('');
      		$('#centrocosto').hide('fast');
      		$('#ccs').html('');
...
<form class="form-inline" role="form" id="frmVer"  method="post" action="?c=solc&a=emisor&id=5&page=1">
				...
				<td>
					...
					<select class="form-control input-sm" id="empresa" name="empresa">
						...
						<option 
							...
							<?php echo ($id_empresa==$emp['id_empresa'] ? 'selected' : '') ?>>
							<?php echo $emp['emp_nombre']; ?>
							</option>
						<?php } ?>
					</select>
					<?php } else { ?>
			...
			<tr>
				...
				<td>
					<div class="form-group">
						...
						<input type="text" class="form-control input-sm"  id="numero" name="numero" value="<?php echo $no_solicitud;?>" />
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div class="form-group">
						...
						<select class="form-control input-sm" id="status" name="status">
							...
							<option value="4" <?php echo ($no_estado=='4' ? 'selected' : '');?>>Recibida en Proveeduria</option>
							<option value="5" <?php echo ($no_estado=='5' ? 'selected' : '');?>>En Espera de Cotizacion</option>
							<option value="12" <?php echo ($no_estado=='12' ? 'selected' : '');?>>Pendiente por Ajuste de Presupuesto</option>
							<option value="6" <?php echo ($no_estado=='6' ? 'selected' : '');?>>Orden de Compra Emitida</option>
							<option value="10" <?php echo ($no_estado=='10' ? 'selected' : '');?>>Desistida</option>
						</select>
					</div>
			  		...
			  		<div class="panel-heading">
			  			...
			  			<h4 class="panel-title" data-toggle="collapse" data-parent="accordion" href="#collapseSolcDet<?php echo $solc['id_prehsol']; ?>">
							&nbsp;&nbsp;&nbsp;<i class="icon-hand-down">#<?php echo $solc['prehsol_numero_sol']; ?></i>
			          		...
			          		<?php if(!empty($solc["id_categoria"])): ?>
			          		<p style="float: right;">
			          			<i class='icon-inbox'></i> Categoría: <span class='text-info'><?php echo $solc["nombre_categoria"]?></span>
			          			<?php if(empty($solc["prehsol_aprobacion_categoria"])): ?>
			          				<i class='icon-time' style="font-size: 12px;" title="Pendiente por Autorizador de Categoría"></i> 
		          				<?php elseif($solc['prehsol_estado']==10 && $solc['usr_estado']==$solc['prehsol_aprobacion_categoria_usuario']): ?>
		          					<i class='icon-remove-circle' style="font-size: 12px;color:red;" title="Desistido por Autorizador de Categoría"></i> 
		          				<?php else: ?>
		          					<i class='icon-ok-sign' style="font-size: 12px;color: green;" title="Aprobado por Autorizador de Categoría"></i>
		          				<?php endif;?>
			          		</p>
			          		<?php endif;?>
			          	</h4>
		...
		?>
			<ul class="pagination">
				...
				<li class="<?php if($page == 1) echo 'disabled'; ?>">
					<a href="?c=solc&a=emisor&id=5&page=<?php echo ($page-1); ?>">&laquo;</a>
				</li>
				<?php } ?>
				<?php
				...
				?>
					<li class="<?php if($page == $i) echo 'active'; ?>">
						<a href="?c=solc&a=emisor&id=5&page=<?php echo $i; ?>"><?php echo $i; ?></a>
					</li>
				<?php
				}
				?>
				...
				<li class="<?php if($page == $totalPag) echo 'disabled'; ?>">
					<a href="?c=solc&a=emisor&id=5&page=<?php echo ($page+1); ?>">&raquo;</a>
				</li>
				<?php } ?>
			</ul>
	   <?php } ?>
