<script>
	...
	function CheckAjaxCall(str) {
	  //var str = $('#empresa').val();
	  myUrl = location.protocol + "//" + location.host + '/sics/json.php?c=req&a=listarCc2';
	  $.ajax({
	    type: 'POST',
	    url: myUrl,
	    dataType: 'json',
...
<div class="row">
	<div class="col-sm-8">
		<h4 class="text-blue">Consulta solicitud de cheque por usuario involucrado en trazabilidad</h4>
	</div>
	<div class="col-sm-4">
		<a href='?c=solc&a=VerS' class="btn btn-default">Consultar Solicitud de Compras</a>
	</div>
</div>
<form class="form-inline" role="form" id="frmVer"  method="post" action="?c=solcheque&a=VerS&id=7&page=1">
	...
	<div class="col-md-12">
		<div class="panel-group" id="accordion">
			...
			<?php foreach($solcs as $solc){  ?>
				<?php if(isset($solc['id_solicitud'])) { ?>
				<div class="panel panel-default">
					<div class="panel-heading">
						<a href="http://intranet.impressa.com/report/?s=<?php echo $solc['id_solicitud'];?>" id="imprimir" target="new" class="pull-left" style="margin-right; 20px;"><i class="glyphicon glyphicon-print"></i> PDF </a>
						<h4 class="panel-title" data-toggle="collapse" data-parent="accordion" href="#collapseSolcDet<?php echo $solc['id_solicitud']; ?>">
							&nbsp;&nbsp;&nbsp;<i class="icon-hand-down">#<?php echo $solc['id_solicitud']; ?></i>
							| <i class="icon-calendar"></i><?php echo $solc['fecha_solicitud'].' '.$solc['hora_solicitud']; ?>
							| <i class="icon-comment"></i><?php echo $solc['primero']; ?>
							| <i class="icon-user"></i><?php echo $solc['segundo']; ?>
							| <i class="icon-tags"></i>Estado:<span class="text-info"><?php echo $solc['estado']; ?></span>
						</h4>
					</div>
	...
	<div class="col-md-12">
		...
		<ul class="pagination">
			...
			<li class="<?php if($page == 1) echo 'disabled'; ?>">
				<a href="?c=solcheque&a=VerS&id=7&page=<?php echo ($page-1); ?>">&laquo;</a>
			</li>
			<?php } ?>
			<?php
			...
			<li class="<?php if($page == $i) echo 'active'; ?>">
				<a href="?c=solcheque&a=VerS&id=7&page=<?php echo $i; ?>"><?php echo $i; ?></a>
			</li>
			<?php
				}
				?>
			...
			<li class="<?php if($page == $totalPag) echo 'disabled'; ?>">
				<a href="?c=solcheque&a=VerS&id=7&page=<?php echo ($page+1); ?>">&raquo;</a>
			</li>
			<?php } ?>
		</ul>
		<?php } ?>
...
$(document).ready(function() {
    $("#empresa").val("<?php echo $_POST['empresa']?>");
