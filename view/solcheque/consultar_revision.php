...
<h4 class="text-blue">Listado de Solicitudes de Cheques de Categoría: <?php echo str_pad($perfil->categoria[0]->id,2,"0",STR_PAD_LEFT)." - ".$perfil->categoria[0]->nombre?></h4>
<div class="row" style="margin-bottom: 5px;">
    ...
    <div class="col-sm-3">
        ...
        <select class="form-control input-sm" id="estado" name="estado" style="min-width: 100%;">
            ...
            <option value="true">Autorizadas Categoría</option>
            <option value="false">Desistidas Categoría</option>
        </select>
    </div>
    <div class="col-sm-2">
        <br/>
...
<table class="table table-condensed tablesorter" style="font-size: 12px;">
	...
	<tbody>
		...
		<tr style="font-size: 10px;">
			...
			<td>
                            <a href="?c=solcheque&a=detalle&s=<?php echo $solc->id?>&return=consultar_revision" class="btn btn-sm btn-default" title="Visualizar solicitud">
    ...
    <nav aria-label="Page navigation" class="pull-right">
        <ul class="pagination">
            <li <?php echo ($paginador['pagina_actual']==0 ? "class='disabled'" : "")?>>
                <a href="?c=solcheque&a=consultar_revision&pag=<?php echo ($paginador['pagina_actual']-1)?>&s=<?php echo $id_estado?>&anio=<?php echo $id_anio?>&mes=<?php echo $id_mes?>" aria-label="Previous" title="Anterior">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            <?php for($pag=0;$pag<$paginador['pagina_total'];$pag++):?>
                <li <?php echo ($pag==$paginador['pagina_actual'] ? "class='active'" : "")?>>
                    <a href="?c=solcheque&a=consultar_revision&pag=<?php echo $pag?>&s=<?php echo $id_estado?>&anio=<?php echo $id_anio?>&mes=<?php echo $id_mes?>"><?php echo ($pag+1)?></a>
                </li>
            <?php endfor;?>
            <li <?php echo ($paginador['pagina_actual']>=($paginador['pagina_total']-1) ? "class='disabled'" : "")?>>
                <a href="?c=solcheque&a=consultar_revision&pag=<?php echo ($paginador['pagina_actual']+1)?>&s=<?php echo $id_estado?>&anio=<?php echo $id_anio?>&mes=<?php echo $id_mes?>" aria-label="Next" title="Siguiente">
...
<script>
	$(document).ready(function(){
        ...
        $('.btn-update').click(function(){
            $(this).attr('href','?c=solcheque&a=consultar_revision&s=' + inp_estado.val() + '&anio=' + inp_anio.val() + '&mes=' + inp_mes.val());
        });
        var pjson={
            mes_anio: function(){
