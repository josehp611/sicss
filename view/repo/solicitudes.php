			...
			<?php } ?>
		  	<?php foreach($solcs as $solc){  ?>  				
		    	...
		    	foreach ($solc['estados'] as $est) {
					...
					$esta[$est['prehsol_stat']]['estado'] = $estados[$est['prehsol_stat']];
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-footer" style="text-align: left !important;">
								<h4>Tracking de Solicitud</h4>
		  						<?php
		  						asort($esta); 
		  						echo '<ul class="list-unstyled">';
		  						foreach ($esta as $estat) {
		  							if(!empty($estat['usuario'])){
		  								echo '<li><b>'.$estat['estado'].'</b> => por '.$estat['usuario'].' en fecha '.$estat['fecha'].' a las '.$estat['hora'].'</li>';
		  							}
		  						} 
		  						echo '</ul>';
		  						?>
