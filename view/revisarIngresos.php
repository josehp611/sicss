...
$(document).ready(function(){
	...
	$("#btnVolver").click(function(){
		document.location='?c=inv&a=inicios';
	});
	$("#btnIngresa").click(function(){
		...
		function(json) {
			if(json == "OK"){
				...
				myUrlOC2 = location.protocol + "//" + location.host + '/sics/json.php?c=inv&a=marcaOCS';
				$.post(myUrlOC2, { 
						empresa: empresa,
				...
				function(data, textStatus){
					...
					if(data == 'OK') {
						...
						$('#detalle tbody tr').each(function (j) {
							...
							campos2['id_prehreq'] = '0';
							campos2['id_prehsol'] = that.find('td').eq(6).text();
...
<table id="encabezado2" class="table table-condensed">
	...
	<tr>
		<td><b><?php echo $Det_Orden['header'][0]['prehsol_usuario']; ?></b></td>
		<td><b>&nbsp;</b></td>
		<td><b><?php echo $Det_Orden['proveedor'][0]['prov_dias']; ?></b></td>
		<td><b><?php echo $Det_Orden['header'][0]['prehsol_numero_sol']; ?></b></td>
	</tr>
</table>
	...
	?>
				...
				?>
							<tr class="danger">
								...
								<td><?php echo $value['predsol_cantidad_aut']; ?></td>
								<td>
									<?php echo $value['predsol_descripcion']; ?>
									<small class="text-danger">* no existe en inventario</small>
								</td>
								<td><?php echo $value['ic_cc']; ?></td>
								<td><?php echo number_format($value['predsol_prec_uni'], 2); ?></td>
								<td><?php echo number_format($value['predsol_total'], 2); ?></td>
								<td><?php echo $value['id_prehsol']; ?></td>
							</tr>
							<?php
							} else {
							?>
							<tr>
								<td><?php echo $value['prod_codigo']; ?></td>
								<td><?php echo $value['predsol_cantidad_aut']; ?></td>
								<td><?php echo $value['predsol_descripcion']; ?></td>
